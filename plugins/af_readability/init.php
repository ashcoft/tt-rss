...
class Af_Readability extends Plugin {
                ...
                if ($tmp && mb_strlen($tmp) < 1024 * 500) {
                        ...
                        try {
                                ...
                                $r = new Readability(new Configuration([
                                        ...
                                        'OriginalURL'          => $url,
                                        'ExtraIgnoredElements' => ['template', 'script', 'style', 'noscript', 'iframe', 'form'],
                                ]));
        ...
        /**
         * Preprocess DLE (DataLife Engine) content to clean up special tags and ad injections
         * @param string $content
         * @return string
         */
        private function preprocess_dle_content(string $content) : string {
                // Remove DLE engine image tags: <!--dle_image_begin:...--><!--dle_image_end-->
                $content = preg_replace('/<!--\s*dle_image_begin:.*?-->/si', '', $content);
                $content = preg_replace('/<!--\s*dle_image_end\s*-->/si', '', $content);

                // Remove common ad injection patterns
                $content = preg_replace('/<\!\-\-(\s*ad\s*)?\-\->/i', '', $content);

                // Remove mgid widget injections
                $content = preg_replace('/<div data-type="_mgwidget".*?<\/div>/si', '', $content);
                $content = preg_replace('/<script>\(function\(w,q\)\{w\[q\]=w\[q\]\|\|\[\];w\[q\]\.push\(\["_mgc\.load"\]\)\}\)\(window,"_mgq"\);<\/script>/si', '', $content);

                // Remove LiveInternet counter script
                $content = preg_replace('/<!--LiveInternet counter-->.*?<!--\/LiveInternet-->/si', '', $content);

                // Remove Facebook SDK
                $content = preg_replace('/<div id="fb-root"><\/div>.*?fb-messagesdk.*?<\/script>/si', '', $content);

                // Remove Google Analytics and similar tracking scripts
                $content = preg_replace('/<script[^>]*>(google|yandex|analytics|counter)\.ru.*?<\/script>/si', '', $content);

                // Remove jsc.mgid ad script
                $content = preg_replace('/<script src="https:\/\/jsc\.mgid\.com.*?<\/script>/si', '', $content);
